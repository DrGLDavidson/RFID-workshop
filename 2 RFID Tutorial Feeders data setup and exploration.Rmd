...
#you should now see df123 in your environment and there should be 1526 observations
...
path<-"F:/Work/www/sunhearts"  ###insert your own path - this is the main folder that contains all the feeder files for each date, but within the specific feeder type (e.g. sunhearts). 
RTfiles<-list.files(path, pattern = "RT", recursive = TRUE) ###create a list of all the files in that folder and subfolders (recursive =T) that contain "RT" in the filename
Sunhearts<-lapply(RTfiles, read.delim)  ###upload all those files into R
class(Sunhearts)  ##it uploads as a list which isn't helpful yet
df <- dplyr::bind_rows(Sunhearts)  ###this binds all the files into a dataframe
Sunhearts<-cbind(df, feeder='sunhearts')  ###add a column called "sunhearts" so you know that comes from the sunheart treatment, and in case you want to bind  sunhearts, gravel and peanut into a single dataframe.  
write.table(Sunhearts, file = "Sunhearts_Allfiles.txt",sep="\t",row.names=FALSE)  

##repeat for Peanut and Gravel.
SPG<-bind_rows(Sunhearts,Peanut,Gravel) ##optional combine them into a single dataframe. 

```

### 2.2.5 Saving your work and returning to it later
```{r}
...
#when you close R, it may ask you if you want to save the global environment. This means you can open that saved file with the data and values as you left off. You can select no if that's not relevant. But if you were in the middle of something and needed to stop, that is a useful option. 
