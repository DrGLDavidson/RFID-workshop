...
#it may help to eyeball the dataframe to see whether 3 seconds has another bird between, or if it's usually the same tag

dfx<-df4%>%
  arrange(dateTime, feeder,  timeSincePreviousVisit)%>%
  select(dateTime, timeSincePreviousVisit, TagID_hex, feeder)

#there are instances when 2 seconds in the same bird, 3 seconds in the same bird, 3 seconds is between birds, even higher numbers are the same bird. We need a code to collapse into a single visit that is not simply removing reads that are within X number of seconds. Instead it needs to be if the previous bird was the same bird AND it's below a given value. 
...
#to ensure we do not loose these visits - i think????
df6<-df4%>%
...
min(df7$timeSincePreviousVisit, na.rm = TRUE) #shortest time between visits
max(df7$timeSincePreviousVisit, na.rm = TRUE) #longest time between visits - but this could be a single bird that went away for a few days
mean(df7$timeSincePreviousVisit, na.rm = TRUE) 
median(df7$timeSincePreviousVisit, na.rm = TRUE)
#how many seconds in a day?
24*60*60
#daylight hours? #usually this experiment occurs in winter, so around 7am to 4pm (9 hours)
9*60*60
#night hours? 
15*60*60
#
